...
# ============================================================
# ✅ 적정 월세 산정식: 구프리미엄 + 회귀계수 + 판정 + Top5 요인
# ============================================================
GU_PREMIUMS = {
    '오피스텔': {
...
FEATURE_AVERAGES = {
    "오피스텔": {"보증금": 3000.0, "전용면적": 28.0, "건축년도": 2014, "층": 8.0, "역_접근성": 0.5},
    "아파트":   {"보증금": 20000.0, "전용면적": 75.0, "건축년도": 2010, "층": 12.0, "역_접근성": 0.4},
    "연립다세대":{"보증금": 8000.0, "전용면적": 45.0, "건축년도": 2012, "층": 3.0, "역_접근성": 0.6},
}
...
def baseline_rent(housing_type: str) -> float:
    avg = FEATURE_AVERAGES[housing_type]
    return predict_rent(
        housing_type=housing_type,
        deposit=float(avg["보증금"]),
        area=float(avg["전용면적"]),
        year=int(avg["건축년도"]),
        floor=int(avg["층"]),
        station_idx=float(avg["역_접근성"]),
        gu="강남구",
    )

def factor_contributions_vs_avg(housing_type: str, deposit: float, area: float, year: int, floor: int, station_idx: float, gu: str):
    c = COEFFICIENTS[housing_type]
    avg = FEATURE_AVERAGES[housing_type]

    contrib = []
    premium = GU_PREMIUMS[housing_type].get(gu, 0.0)
    contrib.append(("구 프리미엄", float(premium)))

    contrib.append(("보증금", float(c["보증금"] * (deposit - avg["보증금"]))))
    contrib.append(("전용면적", float(c["전용면적"] * (area - avg["전용면적"]))))
    contrib.append(("건축년도", float(c["건축년도"] * (year - avg["건축년도"]))))
    contrib.append(("역 접근성", float(c["역_접근성"] * (station_idx - avg["역_접근성"]))))
    contrib.append(("층", float(c["층"] * (floor - avg["층"]))))

    if c.get("면적_x_역", 0) != 0:
        user_inter = area * station_idx
        avg_inter = float(avg["전용면적"]) * float(avg["역_접근성"])
        contrib.append(("면적×역접근(상호작용)", float(c["면적_x_역"] * (user_inter - avg_inter))))

    return contrib

def top5_factors_vs_avg(housing_type: str, deposit: float, area: float, year: int, floor: int, station_idx: float, gu: str):
    contrib = factor_contributions_vs_avg(housing_type, deposit, area, year, floor, station_idx, gu)
    return sorted(contrib, key=lambda x: abs(x[1]), reverse=True)[:5]
...
with st.sidebar.expander("① 기본 선택", expanded=True):
    ...
    if st.session_state.page != "서울 전체 요약":
        ...
        selected_gu = st.selectbox("구 선택", options=all_gu, index=all_gu.index(init_gu))
...
with st.sidebar.expander("② 세부 필터", expanded=(page != "서울 전체 요약")):
    ...
    default_type = [t for t in all_type if "월세" in str(t)] or all_type
    selected_type = st.multiselect("전월세 구분 (실제 데이터는 대부분 월세)", options=all_type, default=default_type)
    ...
    if "건축년도" in df.columns:
        ...
        year_range = st.slider("건축년도 범위", min_value=min_year, max_value=max_year, value=(min_year, max_year))
    else:
        year_range = None
...
def apply_common_filters_cached(
    ...
    if area_range is not None and "전용면적(㎡)" in df_out.columns:
        df_out = df_out[(df_out["전용면적(㎡)"] >= area_range[0]) & (df_out["전용면적(㎡)"] <= area_range[1])]

    if year_range is not None and "건축년도" in df_out.columns:
        df_out = df_out[(df_out["건축년도"] >= year_range[0]) & (df_out["건축년도"] <= year_range[1])]
...
def build_user_condition_text(housing_type, gu, dong, area_range, year_range, only_renew):
    lines = []
    lines.append(f"- 주택유형: {housing_type if housing_type != '전체' else '전체'}")
...
def build_condition_scenario_text(scenarios: list[dict]) -> str:
    ...
    for s in scenarios:
        ...
        if s.get("examples_df") is not None and not s["examples_df"].empty:
            ex = s["examples_df"].head(3)[[c for c in ["구", "동", "전용면적(㎡)", "보증금(만원)", "월세금(만원)"] if c in s["examples_df"].columns]]
...
def cached_run_recommendation_report(user_condition_text: str, candidates_text: str, extra_instruction: str) -> str:
    ...
    return run_recommendation_report(user_condition_text=user_condition_text, candidates_text=candidates_text, extra_instruction=extra_instruction)
...
def cached_run_condition_coach_report(user_condition_text: str, scenario_text: str, extra_instruction: str) -> str:
    ...
    return run_condition_coach_report(user_condition_text=user_condition_text, scenario_text=scenario_text, extra_instruction=extra_instruction)
...
def cached_run_comparison_report(comparison_text: str, extra_instruction: str) -> str:
    ...
    return run_comparison_report(comparison_text=comparison_text, extra_instruction=extra_instruction)
...
def cached_run_market_rarity_report(rarity_text: str, extra_instruction: str) -> str:
    ...
    return run_market_rarity_report(rarity_text=rarity_text, extra_instruction=extra_instruction)

@st.cache_data(show_spinner=False)
def cached_run_listing_judgement_report(listing_text: str, extra_instruction: str) -> str:
    # ✅ 누락됐던 함수: crewai_reports 쪽에 있어야 함
    from crewai_reports import run_listing_judgement_report
    return run_listing_judgement_report(listing_text=listing_text, extra_instruction=extra_instruction)

# =========================
# 매물 정량 평가
# =========================
def evaluate_listing_vs_market(
    df_all: pd.DataFrame,
    housing_type: str,
    gu: str,
    area: float,
    ...
    comp = df_all[
        (df_all["주택유형"] == housing_type) &
        (df_all["구"] == gu) &
        (df_all["전용면적(㎡)"].between(area - 10, area + 10)) &
        (df_all["건축년도"].between(year - 5, year + 5))
    ].copy()

    comp = comp.dropna(subset=["월세금(만원)", "보증금(만원)", "전용면적(㎡)"])

    if len(comp) < 10:
        return {"error": "비교 가능한 유사 매물이 충분하지 않습니다. (표본 10건 이상 권장)"}

    rent_percentile = (comp["월세금(만원)"] < rent).mean() * 100
    unit_price = rent / area
    avg_unit_price = (comp["월세금(만원)"] / comp["전용면적(㎡)"]).mean()

    y_pred = predict_rent(housing_type, deposit, area, year, floor, station_idx, gu)
    diff_model = rent - y_pred

    if rent_percentile <= 30 and diff_model <= 0:
        verdict = "좋은 매물"
    elif rent_percentile >= 70 and diff_model > 0:
        verdict = "비추천 매물"
    else:
        verdict = "보통 수준"
    ...
    return {
        "비교표본수": len(comp),
        "월세_퍼센타일": round(rent_percentile, 1),
        "면적당월세": round(unit_price, 2),
        "비교집단_평균_면적당월세": round(avg_unit_price, 2),
        "모델예측월세": round(y_pred, 1),
        "예측대비차이": round(diff_model, 1),
        "판정": verdict,
    }

def build_listing_evaluation_text(result: dict) -> str:
    if "error" in result:
        return result["error"]

    lines = [
        "[실제 본 매물 vs 시장 비교 결과]",
        f"- 비교 표본 수: {result['비교표본수']}건",
        f"- 월세 퍼센타일: 하위 {result['월세_퍼센타일']}%",
        f"- 면적당 월세: {result['면적당월세']} (비교집단 평균 {result['비교집단_평균_면적당월세']})",
        f"- 모델 예측 월세: {result['모델예측월세']}만원",
        f"- 예측 대비 차이: {result['예측대비차이']}만원",
        f"- 정량 판정: {result['판정']}",
    ]
    return "\n".join(lines)
...
if page == "서울 전체 요약":
    ...
    summary = (
        df_rent.groupby("구")
        .agg(
            거래수=("NO", "count") if "NO" in df_rent.columns else ("월세금(만원)", "count"),
            평균보증금=("보증금(만원)", "mean"),
            평균월세=("월세금(만원)", "mean"),
            평균가격=("가격", "mean") if "가격" in df_rent.columns else ("월세금(만원)", "mean")
        )
        .reset_index()
    )
...
# ============================================================
    ...
    if len(filtered) == 0:
        ...
        with col2:
            st.metric("평균 보증금 (만원)", f"{rent_df['보증금(만원)'].mean():,.0f}")
        with col3:
            st.metric("평균 월세 (만원)", f"{rent_df['월세금(만원)'].mean():,.0f}")
        ...
        if data.empty:
            ...
            view = st.radio("표현 방식 선택", ["히스토그램", "박스플롯", "Q-Q Plot (고급)"], index=0, horizontal=True)
            ...
            if view == "히스토그램":
                ...
                if st.button("히스토그램 PNG 생성/다운로드", key="btn_hist_png"):
                    ...
                    buf.seek(0)
                    st.download_button("Download Histogram (PNG)", data=buf, file_name=f"{loc_label}_rent_histogram.png", mime="image/png")

            elif view == "박스플롯":
                fig = safe_boxplot(series=rent_df["월세금(만원)"], title=f"{loc_label} Monthly Rent Boxplot", ylabel="Monthly Rent (10k KRW)")
                if fig is not None:
                    st.pyplot(fig)
                    if st.button("박스플롯 PNG 생성/다운로드", key="btn_box_png"):
                        ...
                        fig.savefig(buf, format="png", bbox_inches="tight")
                        buf.seek(0)
                        st.download_button("Download Boxplot (PNG)", data=buf, file_name=f"{loc_label}_rent_boxplot.png", mime="image/png")
                ...
                if len(data) < MIN_RENT_FOR_DIST:
                    ...
                    fig, ax = plt.subplots()
                    (theoretical_q, ordered_vals), (slope, intercept, r) = stats.probplot(data.values, dist="norm", fit=True)
                    ...
                    if st.button("Q-Q Plot PNG 생성/다운로드", key="btn_qq_png"):
                        ...
                        fig.savefig(buf, format="png", bbox_inches="tight")
                        buf.seek(0)
                        st.download_button("Download Q-Q Plot (PNG)", data=buf, file_name=f"{loc_label}_rent_qqplot.png", mime="image/png")
    ...
    if len(base) == 0 or len(rent_base) == 0:
        ...
        with tab1:
            ...
            if len(t1) == 0:
                ...
                anomalies_t1 = t1[t1["보증금대비월세비율"] >= threshold].copy().sort_values("보증금대비월세비율", ascending=False)
            ...
            if not needed.issubset(base.columns):
                ...
                if len(t2) == 0:
                    ...
                    if len(base_pos) == 0:
                        ...
                        anomalies_t2 = base_pos[base_pos["월세인상률(%)"] >= thr2].copy().sort_values("월세인상률(%)", ascending=False)
        ...
        with tab4:
            ...
            if len(data) < MIN_RENT_FOR_DIST:
                ...
                st.download_button("IQR 기준 이상치 CSV 다운로드", iqr_outliers.to_csv(index=False).encode("utf-8-sig"),
                ...
                st.download_button("Q-Q Plot 기준 이상치 CSV 다운로드", qq_outliers.to_csv(index=False).encode("utf-8-sig"),
...
""")
    ...
    if calc:
        ...
        base = baseline_rent(htype)
        contrib_all = factor_contributions_vs_avg(
            htype,
            float(deposit),
            float(area),
            int(year),
        ...
        contrib_sum = float(np.sum([v for _, v in contrib_all]))
        y_explain = base + contrib_sum

        st.subheader("1) 적정 월세 결과")
        st.caption(
            f"기준점(서울 평균 {htype}) 월세: **{base:.1f}만원**  +  "
            f"(평균 대비 기여도 합: **{contrib_sum:+.1f}만원**)"
        )
        ...
        if htype == "오피스텔":
            ...
            st.success(
                f"해당 {htype}의 적정 월세는 **약 {lo:.1f}만원 ~ {hi:.1f}만원** 범위로 추정됩니다. (±{band:.1f})"
            )

        st.caption(f"(검증용) 기준점+기여도 합 = {y_explain:.1f}만원, 예측값 = {y_pred:.1f}만원")
        ...
        st.subheader("3) 요인분석 Top5 (서울 평균 대비)")
        factors = top5_factors_vs_avg(
            htype,
            float(deposit),
            float(area),
            int(year),
        ...
        avg = FEATURE_AVERAGES[htype]
        for name, val in factors:
            sign = "+" if val >= 0 else "-"
            if name == "보증금":
                msg = f"(평균 {avg['보증금']:.0f} 대비 {float(deposit)-avg['보증금']:+.0f})"
            elif name == "전용면적":
                msg = f"(평균 {avg['전용면적']:.1f}㎡ 대비 {float(area)-avg['전용면적']:+.1f}㎡)"
            elif name == "건축년도":
                msg = f"(평균 {avg['건축년도']}년 대비 {int(year)-avg['건축년도']:+d}년)"
            elif name == "층":
                msg = f"(평균 {avg['층']:.0f}층 대비 {int(floor)-int(avg['층']):+d}층)"
            elif name == "역 접근성":
                msg = f"(평균 {avg['역_접근성']:.2f} 대비 {float(station_idx)-avg['역_접근성']:+.2f})"
            else:
                msg = ""
            st.write(f"- {name}: **{sign}{abs(val):.1f}만원** 기여 {msg}")

        st.caption("※ Top5는 ‘(사용자값 - 평균값)’ 기반의 기여도(계수×차이)입니다.")
        ...
        if "역접근성지수" not in rent_all.columns:
            ...
            if len(rentx) < MIN_RENT_FOR_DIST:
                ...
                if len(w1) < MIN_RENT_FOR_BASIC or len(w0) < MIN_RENT_FOR_BASIC:
                    st.info(f"각 그룹의 표본 수가 너무 적습니다. (각 그룹 최소 {MIN_RENT_FOR_BASIC}건 권장)")
                else:
    ...
    if len(rent_base) < MIN_RENT_FOR_CLUSTER:
        ...
        with tab_k1:
            ...
            if len(data_k) < MIN_RENT_FOR_CLUSTER:
                ...
                meta_cols = ["주택유형", "구", "동", ("단지명" if "단지명" in rent_base.columns else "건물명"), "계약년월"]
                meta_cols = [c for c in meta_cols if c in rent_base.columns]
                ...
                st.write("#### 📊 클러스터별 평균값")
                st.dataframe(data_k.groupby("cluster").mean(numeric_only=True))
        ...
        with tab_k2:
            ...
            if len(anom_only) < MIN_RENT_FOR_CLUSTER:
                ...
                if len(data_k2) < MIN_RENT_FOR_CLUSTER:
                    ...
                    meta_cols2 = ["주택유형", "구", "동", ("단지명" if "단지명" in anom_only.columns else "건물명"), "계약년월"]
                    meta_cols2 = [c for c in meta_cols2 if c in anom_only.columns]

                    result2 = data_k2.merge(
                        anom_only[meta_cols2],
                        left_index=True,
                        right_index=True,
                        how="left"
                    )
                    ...
                    st.write("#### 📊 클러스터별 평균값")
                    st.dataframe(data_k2.groupby("cluster").mean(numeric_only=True))
...
# =========================
    ...
    tab_rec, tab_coach, tab_comp, tab_rarity, tab_eval = st.tabs(
        ["AI 매물 추천", "AI 조건 코치", "AI 지역/유형 비교", "시장 희소성 브리핑", "AI 매물 판정"]
    )
        ...
        if st.button("CrewAI로 매물 추천 리포트 생성"):
            if df_filtered_ai.empty:
                ...
                try:
                    ...
                    st.error("CrewAI 호출 중 오류가 발생했습니다. (OPENAI_API_KEY / crewai_reports 함수 존재 여부 확인)")
                    st.code(str(e))
    ...
    with tab_coach:
        ...
        if st.button("CrewAI로 조건 코칭 리포트 생성"):
            ...
            if selected_gu is not None:
                df_s1 = apply_common_filters_cached(df, selected_housing, selected_gu, "전체", type_tuple, area_range, year_range, only_renew)
            ...
            if area_range is not None and "전용면적(㎡)" in df.columns:
                ...
                df_s2 = apply_common_filters_cached(df, selected_housing, selected_gu, selected_dong, type_tuple, new_area_range, year_range, only_renew)
            ...
            if year_range is not None and "건축년도" in df.columns:
                ...
                df_s3 = apply_common_filters_cached(df, selected_housing, selected_gu, selected_dong, type_tuple, area_range, new_year_range, only_renew)
            ...
            if only_renew:
                df_s4 = apply_common_filters_cached(df, selected_housing, selected_gu, selected_dong, type_tuple, area_range, year_range, False)
            ...
            if not scenarios:
                ...
                try:
                    ...
                    st.error("CrewAI 호출 중 오류가 발생했습니다.")
                    st.code(str(e))
    ...
    with tab_eval:
        st.markdown("### 🏠 실제 본 매물 AI 판정")
        st.caption("✅ 여기서 입력한 값으로 정량 판정 + CrewAI 정성 판정을 실행합니다. (적정월세 계산기 변수에 의존하지 않음)")

        eval_htype = st.selectbox("주택 유형(판정)", ["오피스텔", "아파트", "연립다세대"], index=0, key="eval_htype")
        eval_gu_list = list(GU_PREMIUMS[eval_htype].keys())
        eval_default_gu = "강남구" if "강남구" in eval_gu_list else eval_gu_list[0]
        eval_gu = st.selectbox("구(판정)", eval_gu_list, index=eval_gu_list.index(eval_default_gu), key="eval_gu")

        e1, e2, e3 = st.columns(3)
        with e1:
            eval_deposit = st.number_input("보증금(만원, 판정)", min_value=0.0, value=1000.0, step=50.0, key="eval_deposit")
        with e2:
            eval_offered = st.number_input("월세(만원, 판정)", min_value=0.0, value=60.0, step=1.0, key="eval_offered")
        with e3:
            eval_area = st.number_input("전용면적(㎡, 판정)", min_value=1.0, value=20.0, step=0.5, key="eval_area")

        e4, e5, e6 = st.columns(3)
        with e4:
            eval_year = st.number_input("건축년도(판정)", min_value=1900, max_value=2100, value=2015, step=1, key="eval_year")
        with e5:
            eval_floor = st.number_input("층(판정)", min_value=-5, max_value=200, value=10, step=1, key="eval_floor")
        with e6:
            eval_distance = st.number_input("역까지거리(m, 판정)", min_value=0.0, value=500.0, step=10.0, key="eval_distance")

        eval_station_idx = distance_to_station_idx(eval_distance, decay_m=500.0)
        st.caption(f"✅ 입력 거리 {eval_distance:.0f}m → 역접근성지수 {eval_station_idx:.3f}")

        if st.button("이 매물 평가하기", key="btn_eval_listing"):
            result = evaluate_listing_vs_market(
                df_all_ai,
                eval_htype, eval_gu,
                float(eval_area), float(eval_deposit), float(eval_offered),
                int(eval_year), int(eval_floor), float(eval_station_idx)
            )

            eval_text = build_listing_evaluation_text(result)
            st.code(eval_text)

            if "error" in result:
                st.warning("정량 비교 표본이 부족해서 AI 정성 판정도 생략합니다.")
            else:
                try:
                    with st.spinner("CrewAI가 매물 판정을 작성 중입니다..."):
                        report = cached_run_listing_judgement_report(
                            listing_text=eval_text,
                            extra_instruction="학생/사회초년생 관점에서 냉정하게 평가해줘."
                        )
